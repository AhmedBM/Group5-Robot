...
static void handle_uart1_interrupt(void* context, alt_u32 id)
{
  ...
  if (uart_status & ALTERA_AVALON_UART_STATUS_RRDY_MSK) {
    // Read character from UART
    uart_chr = IORD_ALTERA_AVALON_UART_RXDATA(UART1_BASE);
...
{
  if (mask & UART1)
    alt_irq_register(UART1_IRQ, NULL, handle_uart1_interrupt);
}
...
static void send_message_to_uart(alt_u8 uart_no, alt_u8* message)
{
  ...
  if (uart_base) {
    ...
    for (i = 0; i < message[MSG_F_LENGTH]; i++) {
      ...
      IOWR_ALTERA_AVALON_UART_TXDATA(uart_base, message[i]);
      usleep(5000);
    }
    #ifdef DEBUG_UART_XMIT
      printf("\n");
